...
class ComparisonsScreen extends Component {
  ...
  grabInfo(mood, feeling) {
    axios
      .get(
        `http://ec2-3-14-132-111.us-east-2.compute.amazonaws.com/api/entry?mood=${mood}&feeling=${feeling}`
      )
      .then(response => {
        let exercise = 0;
        let work = 0;
  ...
  render() {
    ...
    const medicalAttributes = [
      { key: 0, label: "Sic - In Lots of Pain" },
    ...
    return (
      <>
        <View style={styles.container}>
          ...
          />
          {this.state.mood === "" ? null : this.state.mood === "Physical" ? (
            <PhysicalTracker setFeeling={this.setFeeling} />
          ) : this.state.mood === "Medical" ? (
            <MedicalTracker setFeeling={this.setFeeling} />
          ) : this.state.mood === "Emotional" ? (
            <EmotionalTracker setFeeling={this.setFeeling} />
          ) : (
            <MentalTracker setFeeling={this.setFeeling} />
          )}
